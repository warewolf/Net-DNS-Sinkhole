...
is( $trie->add('example.com'),2, 'Trie scalar add scalar context returns number of elements added' );
...
is( $trie->add(qw(yahoo.com google.com bing.com)),6, 'Trie list add scalar context returns number of elements added' );
...
is_deeply( [$trie->add('mtfnpy.com')] , ["*.mtfnpy.com","mtfnpy.com"] , 'Trie basic add list context return values are reversed' );
is_deeply( [$trie->add('ZGSACL.com')] , ["*.zgsacl.com","zgsacl.com"] , 'Trie basic add list context return values are reversed and quashed' );
...
ok ( $trie->lookup_data('example.com') == $ref, 'Trie added data retrieval single');

$trie->add_data('example.net' => $ref, 'example.us' => $ref);
ok ( $trie->lookup_data('example.net') == $ref, 'Trie added data retrieval list A');
ok ( $trie->lookup_data('example.us') == $ref, 'Trie added data retrieval list B');
